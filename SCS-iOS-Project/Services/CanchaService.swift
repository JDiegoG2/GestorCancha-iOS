...
//    func guardarCancha(cancha: CanchaResponse, completion: @escaping (Result<CanchaErrorResponse, AFError>) -> Void) {
...
//               .responseDecodable(of: CanchaErrorResponse.self) { response in
//                   NetworkManager.shared.handleResponse(response: response, completion: completion)
//               }
//       }
//    
//    // Método para consultar una cancha por ID
//    func consultarCancha(id: Int, completion: @escaping (Result<CanchaErrorResponse, AFError>) -> Void) {
...
//    func actualizarCancha(id: Int, cancha: CanchaResponse, completion: @escaping (Result<CanchaErrorResponse, AFError>) -> Void) {
...
//    func listarCanchas(completion: @escaping (Result<[CanchaErrorResponse], AFError>) -> Void) {
...
class CanchaService {
    ...
    // Método para guardar una cancha
    func guardarCancha(cancha: CrearCanchaRequest, completion: @escaping (Result<CanchaResponse, AFError>) -> Void) {
           let parameters: [String: Any] = [
               "tipo_cancha": cancha.tipoCancha.rawValue,
               "numero": cancha.numero,
               "precio": cancha.precio,
    ...
    // Método para actualizar una cancha
    func actualizarCancha(id: Int, cancha: CrearCanchaRequest, completion: @escaping (Result<CanchaResponse, AFError>) -> Void) {
        guard let parameters = convertToParameters(cancha) else {
            let encodingError = NSError(domain: "", code: -1, userInfo: [NSLocalizedDescriptionKey: "Error al codificar los parámetros."])
            completion(.failure(AFError.parameterEncodingFailed(reason: .jsonEncodingFailed(error: encodingError))))
            return
